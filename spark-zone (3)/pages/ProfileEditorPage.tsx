...
const ProfileEditorPage: React.FC<ProfileEditorPageProps> = ({ currentUser, onSave, onExit }) => {
            ...
            <div className="bg-gray-900/50 border border-violet-500/30 rounded-lg p-6 max-w-2xl mx-auto space-y-6">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-4">
                        <div onClick={() => avatarInputRef.current?.click()} className="group relative w-24 h-24 rounded-full cursor-pointer bg-cover bg-center" style={{ backgroundImage: `url(${formData.avatarUrl})` }}>
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value={formData.name || ''} onChange={handleInputChange} className="w-full bg-gray-800/60 border border-violet-500/30 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500" />
                </div>

                <div>
                    <label htmlFor="bio" className="block text-sm font-medium text-gray-300 mb-2">Bio</label>
                    <textarea id="bio" name="bio" value={formData.bio || ''} onChange={handleInputChange} rows={4} className="w-full bg-gray-800/60 border border-violet-500/30 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-none" />
                </div>
            </div>

            <div className="mt-8 pt-6 text-center">
                <button onClick={() => onSave(formData)} className="px-6 py-3 bg-cyan-500 text-white font-semibold rounded-full shadow-lg shadow-cyan-500/30 hover:bg-cyan-400 transform hover:-translate-y-0.5 transition-all duration-300 ease-in-out">
                    Save Changes
                </button>
            </div>
        </div>
