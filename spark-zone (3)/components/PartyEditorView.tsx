...
const StageSetupSection: React.FC<{ partyData: Party; setPartyData: React.Dispatch<React.SetStateAction<Party>> }> = ({ partyData, setPartyData }) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                if (typeof reader.result === 'string') {
                    setPartyData(prev => ({
                        ...prev,
                        stage: { ...prev.stage, tabletop: { ...prev.stage.tabletop, mapUrl: reader.result as string, pinnedCharacterIds: prev.stage.tabletop?.pinnedCharacterIds || [] } }
                    }));
                }
            };
            reader.readAsDataURL(file);
    ...
    const modes = [
        ...
        { id: 'tabletop', label: 'Tabletop', icon: <MapIcon />, desc: 'Interactive map & tokens' },
