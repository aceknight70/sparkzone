...
const WorldSettingsSection: React.FC<WorldSettingsSectionProps> = ({ worldData, setWorldData }) => {
    ...
    const updateRole = (roleId: number, updates: Partial<WorldRole>) => {
        ...
        // Sync selectedRole state if needed (though simple re-render handles it usually, deep update might be safer)
        if (selectedRole?.id === roleId) {
                ...
                {activeTab === 'safety' && (
                    <div className="space-y-6 animate-fadeIn">
                        ...
                        <ContentRatingSelector 
                            ...
                            setRating={(r) => setWorldData(p => ({...p, contentMetadata: {...(p.contentMetadata || { warnings: [] }), ageRating: r }}))}
                            warnings={worldData.contentMetadata?.warnings || []}
                            setWarnings={(w) => setWorldData(p => ({...p, contentMetadata: {...(p.contentMetadata || { ageRating: 'Everyone' }), warnings: w }}))}
                        />
